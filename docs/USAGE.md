...
- 在设置页“教务凭据”区域保存学工号和“邮箱密码”
- 邮箱账号固定为“学工号@sspu.edu.cn”，应用会根据已保存学工号自动派生
...
教务中心页提供“第二课堂学分”卡片，用于通过学工报表系统只读读取第二课堂逐项得分明细。卡片展示得分记录数量和部分项目得分，不再将单项分值相加为总学分。
...
教务中心页新增“本专科教务”摘要卡片，课程表能力则通过独立“课表”页面展示。两者共用同一套 EAMS 只读服务与自动刷新配置。

刷新方式：

- 默认不自动访问本专科教务系统，避免未连接校园网或学校 VPN 时进入页面即发起受限请求
- 可点击教务中心摘要卡片右上角刷新图标手动读取当前摘要
- 可点击侧边导航“课表”进入独立课程表页面，并通过“刷新课表”按钮只读拉取当前学期课程
- 可在设置页“自动刷新设置”中开启“本专科教务自动刷新”并选择刷新间隔；开启后进入教务中心或课程表页会主动读取一次，并按设置间隔刷新
- 每次读取前都会重新执行校园网 / VPN 前置检测；不可访问受限校园站点时不进入 OA / EAMS 登录链路

查询流程：

- 使用 `https://oa.sspu.edu.cn/interface/Entrance.jsp?id=bzkjw` 作为 OA 本专科教务入口
- 复用已保存的 OA/CAS Cookie 会话；会话失效时调用现有 OA 登录校验刷新会话
- 业务站点访问 `https://jx.sspu.edu.cn/eams/` 时固定使用 Firefox UA，降低 EAMS 页面兼容性差异带来的误判
- 进入 EAMS 首页后按只读方式识别课表、成绩、考试、培养计划、开课检索和空闲教室入口；缺失入口或页面结构变化时会降级展示
- 培养计划完成情况通过培养计划课程与已通过成绩做关联统计，不执行计划调整、课程替代申请或确认操作

只读限制：

- 页面只展示个人基本信息、课程名称、时间、地点、教师、周次、成绩、考试时间地点、培养计划和入口状态
- 不提供选课、退课、调课、教学评价、确认、提交申请、预约教室或任何写入入口
- 错误信息不会展示 OA 密码、Cookie、CAS Ticket、Token 或个人详细业务数据

已知限制：

- 开课检索与空闲教室当前已在服务层按只读表单方式支持，并在摘要卡片中展示入口识别状态；后续如需更完整的交互式筛选页面，可在此基础上继续扩展
- 若 EAMS 页面结构调整，摘要卡片和课程表页会展示明确错误状态，不会伪造课表、成绩或考试结果
