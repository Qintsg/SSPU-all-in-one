...
**当前状态说明**：
- 体育部课外活动考勤：使用独立体育部查询密码，只读读取早操、课外活动、次数调整、体育长廊统计与明细
- 第二课堂学分：通过 OA/CAS 登录态进入学工报表系统，只读读取第二课堂逐项得分明细，不将单项分值相加为总学分
- 本专科教务摘要：通过 OA `bzkjw` 入口复用 CAS 会话，聚合展示个人信息、当前课表、成绩、考试、培养计划与培养计划完成情况，并识别开课检索、空闲教室入口状态
- 教学评价等写入型功能仍保持关闭，不在应用内暴露入口
- 受限服务查询前统一执行校园网 / VPN 前置检测，不可达时不提交密码、不刷新 OA 会话
...
**已实现能力**：
- 作为主导航独立页面展示当前学期本专科教务课表
- 展示课程名称、时间、地点、教师、周次等核心字段
- 未配置 OA 密码、未连接校园网 / VPN 或课表不可用时显示明确状态提示
- 支持和教务中心共用同一套本专科教务自动刷新配置
- 仅展示只读课表信息，不提供选课、退课、调课等写入入口
...
**已实现能力**：
- 使用设置页保存的学工号派生学校邮箱账号，并读取独立邮箱密码
- 通过 IMAP `EXAMINE INBOX` + `BODY.PEEK[]` 只读读取最近邮件
- 通过 POP `LIST` + `RETR` 只读读取最近邮件
- 通过 SMTP `EHLO` + `AUTH` 校验登录状态，不发送邮件
- 展示邮件标题、发件人、时间、正文摘要和正文快照
...
- **校园卡余额自动刷新**：默认关闭，可配置主页校园卡余额卡片的自动读取间隔；关闭后仍可在卡片右下角手动刷新并查看上次刷新时间
- **学校邮箱自动刷新**：默认关闭，可配置学校邮箱页面的自动收信间隔；关闭后仍可在邮箱页手动读取最近邮件
- **第二课堂学分自动刷新**：默认关闭，可配置教务中心第二课堂学分卡片的自动读取间隔；关闭后仍可在卡片右上角手动刷新并查看上次刷新时间
- **本专科教务自动刷新**：默认关闭，可配置教务中心本专科教务摘要与独立课程表页面的自动读取间隔；关闭后仍可在对应页面手动刷新
...
- **立即上锁**：不退出应用，直接回到锁定页
- **教务凭据**：保存学工号、OA 密码、体育部查询密码和邮箱密码，学校邮箱账号固定由学工号派生；密码框回访时保持为空，仅显示“已填写 / 未填写”
